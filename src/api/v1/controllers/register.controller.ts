...
export const register = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (employer && !(!!req.body.company_name && req.body.company_name.length >= 3)) {
            return res.status(406).json({message: 'Wrong company name'})
        }
        ...
        bcrypt.hash(password, 10, async (err: Error | undefined, hash: string) => {
            ...
            await pool.query(
                `INSERT INTO "user" (email, password, employer) 
                VALUES ($1, $2, $3) 
                RETURNING id`,
                [email.toLowerCase(), hash, employer],
                async (err, result) => {
                    if (err) {
                        return res.status(500).json({message: 'Unable to create account 1'})
                    } else {
                        if (employer) {
                            await pool.query(
                                ...
                                (err: Error) => {
                                    if (err) {
                                        return res.status(500).json({message: 'Unable to create account 2'})
                                    } else {
